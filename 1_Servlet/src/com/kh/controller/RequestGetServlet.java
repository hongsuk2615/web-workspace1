...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * Get 방식으로 요청했기 때문에 doGet메서드가 호출됨
		 * 
		...
		String[] foods = request.getParameterValues("food"); //체크한 값이 없다면 null이 넘어간다.
		
		if(foods == null) {
			System.out.println("foods : 없음");
		}else {
			System.out.println("foods : " + String.join(",", foods)); //foods.join(",");
			// 배열에 있는 모든 값들을 구분자를 통해서 하나의 문자열로 반환해주는 메서드
		}
		...
		/*
		 * 장점 : Java코드 내에 작성하기 때문에 반복문, 조건문, 유용한 메서드를 사용할 수 있다.
		...
		// 1) 이제부터 내가 출력할 내용은 문서형태이고 html이다. 더불어 문자셋은 utf-8임
		response.setContentType("text/html; charset=UTF-8"); 
		// 2) 응답하고자 하는 사용자(client)와의 스트림(클라이언트와의 통로)을 생성
