...
<body>
	...
	<pre> 
		*변수 선언(&lt;c:set var="변수명" value="리터럴" scope="스코프영역지정(생략가능)"&gt;)
		...
		=> 즉, 해당 scope영역에 setAttribute라는 메서드를 이용해서 key + value형태로 데이터를 담아놓는것과 동일한 형태다
	...
	result 변수값 : ${result } 또는 ${sessionScope.result} <br><br>
	
<!-- 
	변수명만 제시하면 공유범위가 가장 작은곳에서부터 큰곳으로 순차적으로 찾아지게 됨. 티가 나지는 않지만 속도가 조금 느려질수가 있다.
...
-->
	...
	<pre>
		*변수(데이터) 출력 : (&lt;c:out value="출력하고자 하는 값" default="기본값(생략가능)" escapeXml="true/false(기본값:true, 생략가능)"&gt;)
		...
		- escapeXml : 태그로서 해석해서 출력할지 여부(생략가능, 기본값: true)	
	</pre>
	...
	- 조건식을 test속성의 속성값으로 작성(***** 조건식은 무조건 EL 표현식으로 작성해야함 ******)
	...
	<c:choose>
		...
		<c:when test="${num2 eq 2 }">
			<b> num2의 값은 2와 같습니다.</b><br>
		</c:when>
		<c:otherwise>
			<b>num1은 10혹은 2와 일치하지 않습니다.</b><br>
		</c:otherwise>
	...
	<pre>
		for loop문 -(&lt; c:forEach var="변수명" begin="초기값" end="끝값" step="증감값(생략가능)" &gt;)
		향상된 반복문 - (&lt; c:forEach var="변수명" items="순차적으로 접근할 배열, 컬렉션" varStatus="현재접근된요소의 상태값을 저장하는 변수명(생략가능)"&gt;)	
		=> step : 지정하지 않을시 기본값 : 1
	...
	<% 
		...
		list.add(new Person("김홍석0","남","학생0"));
		list.add(new Person("김홍석1","남","학생1"));
		list.add(new Person("김홍석2","남","학생2"));
	...
	<table>
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach var="person" items="${requestScope.list}" varStatus="status">
					<tr align="center">
						<td>${status.count}</td> <!--  1부터 시작하는 값, index : 0부터 시작 -->
						<td>${person.name}</td>
						<td>${person.gender}</td>
						<td>${person.job}</td>
					</tr>
				...
				<c:forEach var="i" begin="0" end="<%= list.size()-1 %>">
					<tr align="center">
						<td>${i+1}</td> <!--  1부터 시작하는 값, index : 0부터 시작 -->
						<td>${list[i].name}</td>
						<td>${list[i].gender}</td>
						<td>${list[i].job}</td>
					</tr>
	...
	-url경로를 생성을 하고, 쿼리스트링을 정의할수 있는 태그
	-넘겨줘야할 쿼리스트링이 길경우 사용하면 편리하고,
	-특정조건에 따라 파라미터값을 다르게 보내야 할때 자주 사용됨
	</pre>
	
	<c:set var="contextPath" value="<%= request.getContextPath() %>"/>
	<a href="${contextPath}/list.do?currentPage=1&num4">기존방식</a> <br>
