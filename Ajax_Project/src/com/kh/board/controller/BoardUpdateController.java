...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// 필요한 데이터를 전달받고 실제 Board테이블과 Attachment테이블에 Update쿼리문 실행

		// 1. 전송된데이터 iput file이 포함된 경우 enctype="multipart/form-data"로 전송했을것.
		if (ServletFileUpload.isMultipartContent(request)) {
			// 1-1 전송파일 용량제한(10Mbyte)
			int maxSize = 1024 * 1024 * 10;
			// 1-2 전달된 파일을 저장시킬 서버 폴더의 물리적인 경로 알아내기
			...
			if (multi.getOriginalFileName("upfile") != null) {
				...
				// 첨부파일이 원래 등록되어있을 경우, 원본파일의 파일번호, 수정된 이름을 hidden으로 넘겨받았음
			...
			// 하나의 트랜젝션으로 board에 update문과 Attachment테이블의 insert, update 동시에 처리해주기
			// 항상 board에 update문은 실행됨.
			int result = new BoardService().updateBoard(b, at); 
			// case1 : 새로운 첨부파일이 없는 경우(x) -> insert(x), update(x)
			// case2 : 새로운 첨부파일이 있는 경우(o), 기존에도 첨부파일이 있던경우(o) -> update(o) insert(x)
			// case3 : 새로운 첨부파일이 있는 경우(o), 기존에는 첨부파일이 없던 경우(x) -> update(x), insert(o)
		
			if(result > 0) {	// 성공시 : 상세조회페이지로 redirect
